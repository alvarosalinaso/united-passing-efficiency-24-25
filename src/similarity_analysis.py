...
def run_similarity(
    ...
    for csv_file in csv_files:
        ...
        for idx, player in enumerate(players):
            ...
            profiles[player]["cluster"] = (
                int(np.argmax(sim_matrix[idx])) if idx != np.argmax(sim_matrix[idx]) else -1
            )
        ...
        results[name] = {
            ...
            "mean_similarity": round(
                float(np.mean(sim_matrix[np.triu_indices(len(players), k=1)])), 4
            ),
            "profiles": profiles,
        }
        print(
            f"[SIM] {name}: {len(players)} jugadores, similaridad media = {results[name]['mean_similarity']:.3f}"
        )
